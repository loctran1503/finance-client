...
import { authSelector, buyCrypto, sellCrypto } from "../../../store/reducers/authSlice";
...
import { Coin, UserResponse } from "../../../utils/types/api";
...
const SellCrypto = ({ id, price }: BuyCryptoType) => {
  Modal.setAppElement("#root");
  const [amount, setAmount] = useState<string>("");
  const { isAuthenticated, user } = useAppSelector(authSelector);
  const [userCoin,setUserCoin] = useState<Coin | null>(null)
  ...
  React.useEffect(() =>{
    if(user){
      const userCoinExisting = user.coins?.find(item => item.coinId===id)
      if(userCoinExisting) setUserCoin(userCoinExisting)
  ...
  const handleSubmit = async () => {
    try {
      ...
      console.log(userCoin!.amount < Number.parseFloat(amount));
      console.log(`userCoin:${userCoin!.amount}`)
      console.log(`amount:${Number.parseFloat(amount)}`)
      
      if (userCoin && userCoin.amount < Number.parseFloat(amount)) {
        setErrorMessage(`Not enough ${id} amount`);
        return;
      }
    ...
    <div>
      ...
      >
        ...
        <div className={styles.container}>
          <h2 className={styles.header}>Sell Cryptocurrency</h2>
          {errorMessage && (
            <h3 className="error-message-header">{errorMessage}</h3>
          )}
          <p className={styles.porfolioAmount}>
            Your {id} amount:{" "}  
            {/* <span>{decimalConverterWithoutCurrency(userCoin?.amount || 0)}</span> */}
            <span>{userCoin?.amount || 0}</span>
          </p>
          <label className={clsx(styles.labelCustom, "label-custom")}>
            Enter {id} amount you want to sell
          </label>
          <input
